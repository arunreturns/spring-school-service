...
import app.db.rowmapper.ClassroomRowMapper;
import app.db.rowmapper.StudentRowMapper;

@Service
public class ClassroomDBOps {
	...
	public List<Student> getStudentsInClassRoomFromDB(Integer classroomId) {
		String query = "SELECT * FROM STUDENTS s, CLASSROOM c WHERE c.classroomId = :classroomId AND s.studentClass = c.classroomName";
		logger.info("Running query " + query);
